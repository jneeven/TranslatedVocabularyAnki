...
portuguese_vocab = json.loads(Path("Vocabs", "portuguese_vocab.json").read_bytes())
untranslated_vocab = Path("Vocabs", "vocab.csv").read_text().strip().splitlines()
...
for key, value in portuguese_vocab.items():
    ...
    # Make sure an entry exists so we don't have to distinguish between words that
    # already existed in Spanish and those that did not.
    if new_key not in combined_dict:
        combined_dict[new_key] = {"full": key, "tags": []}

    # Portuguese vocab has no tags, so the only thing to insert is the translation.
...
sorted_dict = dict(sorted(combined_dict.items(), key=lambda item: item[1]["tags"]))
Path("Vocabs", "combined_vocab.json").write_text(json.dumps(sorted_dict, indent=4))
